...
4. [メイン編集画面](#4-メイン編集画面)
5. [右パネル](#5-右パネル)
6. [縦断線形設計ウィンドウ](#6-縦断線形設計ウィンドウ)（Undo/Redo 含む）
7. [3D走行ビューア](#7-3d走行ビューア)（俯瞰・複数台・オートドライブ 含む）
...
オフセット拘束には2種類ある。いずれも右パネル（5.10 節参照）から設定・解除する。

#### 2.5.1 OffsetConstraint（円 2 個 + 直線 1 本）

**直線 S と 2 つの円 A・B** の組に対して設定し、直線を 2 円からのオフセット距離で拘束する。円が動くと直線が追従する（`Circle → Line`）。
...
| `off_a` | 直線 S と円 A のオフセット量 [m]（正のとき外側、負のとき内側） |
| `off_b` | 直線 S と円 B のオフセット量 [m] |
| `feasible` | 最後の `solve()` が成功した場合 `True`。拘束が成立しない状態（2 円が近すぎる等）では `False` |
...
**法線方向の維持**: 設定時点での各円の `signed_dist`（直線に対して左側が正）の符号を記憶し、以後の `solve()` でも同じ符号の解を選ぶ。これにより「直線が 2 円の間にある」「直線が 2 円の外側にある」などの位置関係が変化しない。
...
**拘束が成立しない場合**: 2 円の距離関係から同時に満たす直線が存在しない場合、`solve()` は直線を変更せず `feasible = False` を設定する。2 円が再び適切な位置に戻ると自動的に追従を再開する。

**操作方法**: 円 2 個と直線 1 本を選択した状態で右パネルに設定パネルが表示される。

#### 2.5.2 TwoLineOffsetConstraint（直線 2 本 + 円 1 個）

**2 本の直線 A・B と円 C** の組に対して設定し、円を 2 直線からのオフセット距離で拘束する。直線が動くと円の中心が追従する（`Line → Circle`）。

| フィールド | 説明 |
|-----------|------|
| `line_a` | 基準となる直線 A への参照 |
| `line_b` | 基準となる直線 B への参照 |
| `circle` | 中心位置が拘束される円 C への参照 |
| `off_a` | 直線 A と円 C のオフセット量 [m]（正のとき外側、負のとき内側） |
| `off_b` | 直線 B と円 C のオフセット量 [m] |
| `feasible` | 最後の `solve()` が成功した場合 `True`。2 直線が平行なとき `False` |

**拘束条件**:

```
直線 A から円 C の中心への垂直距離 = C.radius + off_a
直線 B から円 C の中心への垂直距離 = C.radius + off_b
```

直線 A・B の移動または `off_a`・`off_b`・`C.radius` の変更に追従して円 C の中心が再計算される。

**法線方向の維持**: 設定時点での各直線に対する円中心の `signed_dist` 符号（`_eps_a`・`_eps_b`）を記憶し、以後の `solve()` でも同じ符号の解を選ぶ。

**拘束が成立しない場合**: 2 直線が平行（行列式 ≈ 0）のとき `solve()` は円中心を変更せず `feasible = False` を設定する。2 直線が再び交差する角度に戻ると追従を再開する。

**操作方法**: 直線 2 本と円 1 個を選択した状態で右パネルに設定パネルが表示される。
...
```json
{
  ...
  "clothoids": [
    { "id": 5, "nickname": "clo_1", "line_id": 1, "circle_id": 3,
      "reversed_flag": false, "snap_segment": true, "snap_arc": true,
      "split_seg_ids": [], "split_arc_ids": [] }
  ],
  "offset_constraints": [
    { "id": 7, "line_id": 1, "ca_id": 3, "cb_id": 4, "off_a": 10.0, "off_b": 5.0 }
  ],
  "two_line_offset_constraints": [
    { "id": 8, "la_id": 1, "lb_id": 2, "circle_id": 3, "off_a": 5.0, "off_b": 5.0 }
  ],
  "element_profiles": [
    {
...
ニックネームは各図形定義の `id` フィールドの直後に `nickname` フィールドとして含める（未設定の図形には出力しない）。旧フォーマット（トップレベル `nicknames` フィールド）との後方互換も維持する。

クロソイドの `split_seg_ids`/`split_arc_ids` は snap=off のとき接点分割で生成した線分・円弧の ID リスト。保存・復元することで、読み込み後の `compute()` 再実行時に重複分割を防ぐ。

### 2.8 ID 管理とニックネーム

全図形（`Line`・`Segment`・`Circle`・`Arc`・`Clothoid`・`OffsetConstraint`・`TwoLineOffsetConstraint`・`GradeLine`・`VerticalCurve`）の ID はタイプを通じてユニーク。
...
**ニックネームのデフォルト**: なし。未設定の図形を画面上で表示する必要がある場合は `(type#id)` 形式（例: `(直線#6)`）を使用する。この文字列はファイルには保存されない。

**ID の振り直し**: メニュー「ID を振り直す」を実行すると、全図形の ID を 1 から連番で付け直す。クロソイドやオフセット拘束の参照 ID も追従して更新される。ニックネームキーも新 ID に更新される。

**シーンへの追加読み込み（マージ）**: メニュー「追加で読み込む（`Ctrl+Shift+O`）」でファイルを選択すると、現在のシーンに JSON データを追加読み込みする。追加時は既存の選択を解除し、追加した図形だけを選択状態にする。ID 衝突が発生した場合は後から追加される図形に新 ID を割り当てる。
...
| `Shift` + 左ドラッグ（何もない場所） | ラバーバンド選択（矩形による一括選択。4.5 節参照） |
| 中ボタンドラッグ | パン |
...
**ラバーバンド選択（`Shift` + 左ドラッグ）**:

選択モードで `Shift` を押しながら何もない場所をドラッグすると、矩形による一括選択ができる。

- 矩形に**完全に含まれる**図形のみ選択する（線分: 両端点、円弧: 端点と弧上のサンプル点全部、円（弧なし）: 上下左右 4 点、クロソイド: 全描画点）
- 線分・円弧が選択されるとき、その親の直線・円も一緒に選択される
- ドラッグ中は矩形が青系の破線枠 + 半透明塗りで表示され、対角線のワールド距離が右パネルに「距離: x.xxx m」と表示される（**簡易測距ツール**を兼ねる）
- 矩形サイズが 4px 未満の場合はクリック扱い（選択なし）
- `Esc` キーでドラッグ中のラバーバンド選択をキャンセルできる
...
実効的な選択図形（`effective_set`）が 2 個以上のとき、選択範囲を囲む AABB（Axis-Aligned Bounding Box）枠線と操作ハンドルを表示する。

| ドラッグ対象 | 動作 |
|------------|------|
| 頂点ハンドル（4 隅の丸） | AABB 中心を基点にした XY 等率拡大縮小（X・Y 方向の変化量の大きい方を採用） |
| 辺（4 辺のいずれか） | 選択図形全体を平行移動 |
| 対角線（AABB の対角線上） | AABB 中心を回転中心とした回転移動 |

いずれの操作もドラッグ開始時のスナップショットを基準に変換を計算し（累積誤差なし）、ドラッグ完了時に Undo スタックに記録される。
...
右パネルに OffsetConstraint パネルが表示される（5.10.1 節参照）。スムーズ接続で生成された円は選択不可（警告を表示）。

**2 直線 + 1 円が選択された場合**:

右パネルに TwoLineOffsetConstraint パネルが表示される（5.10.2 節参照）。
...
- 直線・円・クロソイド・線分・円弧の追加・削除、全削除
- ハンドルのドラッグ（ドラッグ開始前に自動でスタックに積む）
- 複数選択時の AABB ドラッグ（平行移動・拡大縮小・回転。ドラッグ開始時に記録）
- 右パネルからのプロパティ変更（X/Y 座標・半径・角度等の数値入力・マウスホイール操作）
- 右パネルからの複数選択操作（コピー・平行移動・回転・拡大縮小）と Paste（始点/終点ペアの貼り付け）
- 折れ線接続・スムーズ接続・接続解除
- クロソイドの追加・削除・反転
- オフセット拘束の設定・解除（両種類）
- 追加で読み込む（マージ、`Ctrl+Shift+O`）

**Undo の対象外**: ニックネームの変更、オフセット量（`off_a`/`off_b`）スピンボックスのリアルタイム編集、ID の振り直し（実行前に確認ダイアログを表示する）。
...
カーソルが図形の上にある（ホバー中）場合、座標の直下にその図形の情報を表示する。

- 表示形式: `{ニックネーム} ({タイプ}#{id})`（ニックネーム未設定の場合は `({タイプ}#{id})` のみ）
- 線分・円弧については親図形のニックネームも表示する

ラバーバンド選択（4.5 節）のドラッグ中は、矩形対角線のワールド距離を「距離: x.xxx m」形式で同エリアに表示する（簡易測距）。ドラッグ終了で表示は消える。
...
`{ニックネーム}` は `display_name(id, type_label)` の戻り値（ニックネーム設定済みなら実名、未設定なら `(type#id)` 形式）。
...
高優先候補のうち以下の条件を満たす場合、その候補の直後に `[道なり] <図形ラベル>` アイテムを自動追加する。

- 高優先候補が1件だけ
- 高優先候補が複数あり、そのうち `[順]` 判定の候補がちょうど1件

`[道なり]` アイテムが選択されると `_road_follow()` が起動し、以降のコンボボックスに対して同じ条件で連鎖的に選択を進める。連鎖中は `[道なり]` 自身を候補に含めない（無限ループ防止）。
...
プロパティの数値入力・マウスホイール操作は Undo に記録される。同一編集セッション（フォーカスを得てから移るまで）での連続変更は 1 手順としてまとめられる。

ハンドルのドラッグ完了時（マウスボタンを離した瞬間）に右パネルのプロパティが即座に更新される。

**Copy / Paste ボタン**（直線・線分のプロパティ画面に表示）:

| ボタン | 動作 |
|--------|------|
| `⧉ Copy` | 現在の始点・終点ペアをクリップボードにコピー |
| `⧈ Paste` | クリップボードの始点・終点ペアを貼り付け（左クリック: そのまま貼り付け、右クリック: 変換オプションメニューを表示） |

Paste の右クリックメニューでは、原点を基準とした以下の変換を選んで貼り付けられる。

- 回転: 90° / 180° / −90°
- 線対称: `y=0`（x 軸）/ `x=0`（y 軸）/ `y=x` / `y=−x`

クリップボードの内容は `{"sx", "sy", "ex", "ey"}` 形式の JSON テキストで管理する。クリップボードに有効なペアがないとき Paste ボタンは無効化され、クリップボードの変更に自動追従して有効化される。Paste の実行は Undo に記録される。

**子の図形リスト**:

- **直線を選択した場合**: 所属する線分を始点の t 順に一覧表示する。各行にニックネーム・始点/終点座標・長さと「選択」ボタンを表示する
- **円を選択した場合**: 所属する円弧を `angle_start` 順に一覧表示する。各行にニックネーム・始点/終点角度・弧長と「選択」ボタンを表示する

**円弧追加ボタン**（円のプロパティ画面に表示）:

円周上に円弧のない空き区間がある場合のみ表示する。

| ボタン | 動作 |
|--------|------|
| 「円弧を追加」 | 空き区間の中で中心角が最大の1本を追加 |
| 「円弧を全追加」 | 空き区間すべてに円弧を追加 |
...
**関連オフセット拘束の一覧**: 選択図形が関与するオフセット拘束（OffsetConstraint / TwoLineOffsetConstraint の両方）も一覧表示する。各拘束行の「選択」ボタンを押すと、その拘束に含まれる全図形が選択され、拘束のプロパティパネル（5.10 節）に切り替わる。

### 5.6 複数図形選択時の操作パネル

実効的な選択図形（`effective_set`）が 2 個以上のとき、右パネルに以下の操作グループが表示される。マウスによる AABB ドラッグ（4.5 節）とは独立して数値入力で操作できる。

**コピー**:

選択図形を複製し、元の選択を外して複製した図形のみ選択状態にする。クロソイドは、参照する直線・円が同時に複製される場合はその複製物を参照する。対応する縦断線形データ（`ElementProfile`、勾配直線・縦断曲線を含む）も複製される。

**平行移動**:

ΔX・ΔY を数値入力し「適用」ボタンで全選択図形を移動する。

**回転**:

角度（度数）と回転中心（AABB 中心 / 原点）を指定して「適用」ボタンで全選択図形を回転する。

**拡大縮小（XY 同率）**:

倍率と基準点（AABB 中心 / 原点）を指定して「適用」ボタンで全選択図形を拡大縮小する。クロソイドのパラメータ整合性を保つため XY 同率のみ対応。
...
### 5.8 再描画（全クロソイド再計算）ボタン

全クロソイドを再計算し、再描画する。

### 5.9 ニックネーム管理

すべての図形（直線・線分・円・円弧・クロソイド）に任意のニックネームを設定できる。ニックネームはファイル保存・読み込みに対応し、縦断線形ウィンドウのカラーバーラベルとしても使用される。

ニックネームを設定しない場合、画面表示には `(type#id)` 形式（例: `(クロソイド#5)`）が使われる。この文字列はファイルには保存されない（`get_nickname()` は `None` を返す）。

### 5.10 オフセット拘束パネル

選択図形の組み合わせに応じて2種類のパネルが表示される。

#### 5.10.1 OffsetConstraint パネル（円 2 個 + 直線 1 本）
...
#### 5.10.2 TwoLineOffsetConstraint パネル（直線 2 本 + 円 1 個）

**拘束が未設定の場合**:
- 2 直線と円のニックネーム、`off_a`・`off_b` のスピンボックス（初期値 0）を表示
- 「オフセット拘束を設定」ボタン: クリック時に現在の位置関係から `off_a`・`off_b` の初期値を自動算出して拘束を登録する

**拘束が設定済みの場合**:
- `off_a`・`off_b` のスピンボックス（リアルタイム編集可能）
- 各直線から円の中心への現在距離と期待値（`R + off`）を情報表示
- 「オフセット拘束を解除」ボタン

`off_a`・`off_b` の変更は即座に円の中心位置に反映される（Undo 非対応、設定・解除操作は Undo 対応）。

#### 循環依存の検出

`OffsetConstraint`（円→直線）と `TwoLineOffsetConstraint`（直線→円）が互いを参照し合うと循環依存が生まれ、解が収束しない。拘束を設定しようとした時点でグラフ探索により循環を検出し、循環が生じる場合は警告ダイアログを表示して拘束の登録を拒否する。
...
- 勾配直線の追加・削除
- ハンドルのドラッグ（ドラッグ開始前に自動でスタックに積む）
- 数値入力によるプロパティ変更（勾配直線の距離・標高）
- 縦断曲線の挿入・削除

Undo スタックはシーン全体の Undo（メイン画面の `Ctrl+Z`）とは独立している。縦断線形ウィンドウの Undo はキャンバスの `ProfileCanvas._undo_stack` で管理し、ウィンドウを閉じると破棄される。
...
`V` キーおよび `O` キーで視点を切り替える。

| モード | 説明 | 切替 |
|--------|------|------|
| `follow` | 自車を後方から追う追従視点（デフォルト） | `V` |
| `onboard` | 車載視点（運転席から前方を見る） | `V` |
| `overview` | 自車を真上から追う俯瞰視点 | `O` |
| `overview_fixed` | カメラ位置を固定した俯瞰視点（右/中ドラッグでパン） | `O` |

`V` は `follow` ↔ `onboard` のみを切り替える。`O` は `follow`/`onboard` → `overview` → `overview_fixed` → `follow` とサイクルする。

`overview` / `overview_fixed` 中は `I` / `K` キーでカメラ高度（ズーム）を変更できる。

### 7.6 複数台走行（トラフィック）

起動時、自車と同じ中心線上に周囲車両（赤い直方体）が一定間隔（`TRAFFIC_GAP = 20 m`）で初期配置される（前方に「道路長 ÷ TRAFFIC_GAP」台）。周囲車両は自車と同じ中心線上を個別の速度係数（±30%）で走行し、端点でループする。

| キー | 動作 |
|------|------|
| `P` | 周囲車両を 1 台追加（前方に配置） |
| `Shift+P` | 周囲車両を 1 台削除（最後に追加した台） |

### 7.7 オートドライブ

`A` キーでオートドライブ ON/OFF を切り替える。オートドライブが有効な場合（3D ビューアが全要素グラフ付きで起動されたとき）、自車は中心線の端点に達すると隣接する次の要素をランダムに選択して走行を継続する。

- 端点で候補が1つなら自動的にそちらへ進む
- 候補が複数の場合はランダムに選択する
- 遷移前の状態は走行履歴スタック（最大 10 要素）に積まれる
- `←` キーで履歴を 1 要素分遡れる（オートドライブ中のみ有効）
- `→` キーで履歴を 1 要素分進める（履歴参照中のみ有効）
- HUD に現在の走行中図形名と履歴参照中インジケータを表示する
...
|------|------|
| `V` | 追従視点 ↔ 車載視点の切替 |
| `O` | 俯瞰視点をサイクル（追従俯瞰 → 固定俯瞰 → 通常に戻る） |
| `I` / `K` | 俯瞰モードでカメラをズームイン / ズームアウト |
| `A` | オートドライブ ON/OFF |
| `P` | 周囲車両を 1 台追加 |
| `Shift+P` | 周囲車両を 1 台削除 |
| `R` | 路面表示 ON/OFF |
...
| オプション | 説明 |
|------------|------|
| `-i FILE` / `--input-file FILE` | 起動時に指定した `.rdjson` ファイルを読み込む |
...
| `Ctrl+Shift+I` | 子図形を初期化して保存（直線の線分を1本化・円弧を全削除・クロソイドのsnap=off にしてから別名保存。元シーンは変更しない） |
| `Ctrl+O` | ファイルを開く |
...
| `Del` | 選択図形を削除 |
| `Esc` | 連続入力をリセット |
| —（メニューのみ） | ID を振り直す（全図形の ID を 1 から連番で付け直す。Undo 非対応のため実行前に確認ダイアログを表示） |
| —（メニューのみ） | 全削除（確認ダイアログ表示後、シーン全データを削除。Undo 対応） |
| —（ビューメニュー、チェック項目） | 右パネルを表示（デフォルト非表示。チェックで表示） |
