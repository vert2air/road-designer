...
オフセット拘束には2種類ある。いずれも右パネル（5.9 節参照）から設定・解除する。
...
右パネルに OffsetConstraint パネルが表示される（5.9.1 節参照）。スムーズ接続で生成された円は選択不可（警告を表示）。

**2 直線 + 1 円が選択された場合**:

右パネルに TwoLineOffsetConstraint パネルが表示される（5.9.2 節参照）。
...
- 複数選択時の AABB ドラッグ（平行移動・拡大縮小・回転。ドラッグ完了時に記録）
...
| `⧈ Paste` | クリップボードの始点・終点ペアを貼り付け（左クリック: そのまま、右クリック: 始点/終点を入れ替えて貼り付け等の変換オプションあり） |

クリップボードの内容は `application/x-road-designer-point-pair` MIME タイプ互換の JSON 形式で管理する。

**子の図形リスト**:

- **直線を選択した場合**: 所属する線分を始点の t 順に一覧表示する。各行に始点・終点の XY 座標・割合 t、および Copy / Paste・削除ボタンを表示する
- **円を選択した場合**: 所属する円弧を `angle_start` 順に一覧表示する。各行に始点・終点の角度と XY 座標、Copy / Paste・削除ボタンを表示する
...
選択図形を複製し、元の選択を外して複製した図形のみ選択状態にする。
...
#### 5.11.1 OffsetConstraint パネル（円 2 個 + 直線 1 本）
...
#### 5.11.2 TwoLineOffsetConstraint パネル（直線 2 本 + 円 1 個）
...
| `Del` | 選択図形を削除 |
| `Esc` | 連続入力をリセット |
| —（メニューのみ） | ID を振り直す（全図形の ID を 1 から連番で付け直す） |
