...
`snap_segment`・`snap_arc` のデフォルトはいずれも `False`。直接生成したクロソイドは snap off（接点で線分・円弧を分割して管理）が基本動作。スムーズ接続（4.6節）で自動生成されるクロソイドのみ両方 `True` で生成する。
...
### 2.5 オフセット拘束（OffsetConstraint）

「オフセット拘束」は**直線 S と 2 つの円 A・B** の組に対して設定し、直線を 2 円からのオフセット距離で拘束するデータ構造。

| フィールド | 説明 |
|-----------|------|
| `line` | 拘束される直線 S への参照 |
| `circle_a` | 円 A への参照（スムーズ接続で生成された円は不可） |
| `circle_b` | 円 B への参照（スムーズ接続で生成された円は不可） |
| `off_a` | 直線 S と円 A の外側オフセット量 [m]（正のとき外側、負のとき内側） |
| `off_b` | 直線 S と円 B の外側オフセット量 [m] |
| `feasible` | 最後の `solve()` が成功した場合 `True`。拘束が成立しない状態（円が近すぎる等）では `False` |

#### 拘束条件

```
直線 S から円 A の中心への垂直距離 = A.radius + off_a
直線 S から円 B の中心への垂直距離 = B.radius + off_b
```

円 A・B の中心移動または半径変更に追従して直線 S が自動更新される。

#### 法線方向の維持

設定時点での各円の `signed_dist`（直線に対して左側が正）の符号を記憶し、それ以後の `solve()` でも同じ符号になる解を選ぶ。これにより「直線が 2 円の間にある」「直線が 2 円の外側にある」などの位置関係が変化しない。

内部フィールド `_eps_a`・`_eps_b`（符号フラグ、`+1` または `-1`）は `calc_offsets_from_current()` で設定される:
...
2 円の距離関係から上記拘束を同時に満たす直線が存在しない場合（例: 2 円が近すぎる）、`solve()` は直線を変更せず `feasible = False` を設定する。2 円が再び適切な位置に戻ると自動的に追従を再開する（`feasible = True`）。

#### 操作方法

円 2 個と直線 1 本を選択した状態で右パネルに「オフセット拘束」パネルが表示される（5.9 節参照）。

### 2.6 ElementProfile（縦断線形データ）
...
```json
{
  ...
  "offset_constraints": [
    { "id": 7, "line_id": 1, "ca_id": 3, "cb_id": 4, "off_a": 10.0, "off_b": 5.0 }
  ],
  "element_profiles": [
    {
...
全図形（`Line`・`Segment`・`Circle`・`Arc`・`Clothoid`・`OffsetConstraint`・`GradeLine`・`VerticalCurve`）の ID はタイプを通じてユニーク。

**保存時**: `to_dict()` の呼び出し前に `_fix_duplicate_ids()` が走り、メモリ上の ID 重複を検出して自動修正する。これにより保存ファイルの整合性を保証する。

**読み込み時**: ファイル読み込み後は `_id_counter` を全 ID の最大値 + 1 から再開して重複を防ぐ。ID 衝突を検出した場合は後から現れた図形に新 ID を割り当てる（`_resolve_id()`）。ID が振り直された場合でも、クロソイドやオフセット拘束が参照する `line_id`・`circle_id` が正しく解決されるようフォールバック参照を保持する。
...
右パネルにオフセット拘束パネルが表示される（5.9 節参照）。スムーズ接続で生成された円は選択不可（警告を表示）。
...
- 直線・円・クロソイド・線分・円弧の追加・削除
- ハンドルのドラッグ（ドラッグ開始前に自動でスタックに積む）
- 右パネルからのプロパティ変更（X/Y 座標・半径・角度等の数値入力）
- 折れ線接続・スムーズ接続・接続解除
- クロソイドの追加・削除・反転
- オフセット拘束の設定・解除
...
1個目のコンボボックスに図形が設定されたとき（設計画面でのクリック選択・コンボ直接操作のいずれでも）、直ちに2個目のコンボボックスの高優先候補（隣接図形）が更新される。選択自体はユーザーが行う。
...
プロパティの数値入力は Undo に記録される。同一編集セッション（フォーカスを得てから移るまで）での連続変更は 1 手順としてまとめられる。

ハンドルのドラッグ完了時（マウスボタンを離した瞬間）に右パネルのプロパティが即座に更新される。
...
円 2 個と直線 1 本が選択された状態で表示される。

**条件**:
- 選択された円がいずれもスムーズ接続で生成された円（`bisector_dir` が設定された円）でないこと。該当する場合は警告を表示して設定不可とする。

**拘束が未設定の場合**:
- 直線と各円のニックネーム、`off_a`・`off_b` のスピンボックス（初期値 0）を表示
- 「オフセット拘束を設定」ボタン: クリック時に現在の位置関係から `off_a`・`off_b` の初期値を自動算出して拘束を登録する

**拘束が設定済みの場合**:
- `off_a`・`off_b` のスピンボックス（リアルタイム編集可能）
- 直線から各円の中心への現在距離と期待値（`R + off`）を情報表示
- 「オフセット拘束を解除」ボタン

`off_a`・`off_b` の変更は即座に直線の位置に反映される（Undo 非対応、設定・解除操作は Undo 対応）。
