...
def _tangent_normal_at(
    centerline: list, i: int
...
def build_car_box(length: float = 4.0, width: float = 2.0,
    ...
    faces = [
        ...
        ([(-hl, -hw, 0), (hl, -hw, 0), (hl, hw, 0), (-hl, hw, 0)],
         (0, 0, -1), LColor(0.7, 0.1, 0.1, 1)),  # 底
        ([(-hl, -hw, hh), (hl, -hw, hh), (hl, hw, hh), (-hl, hw, hh)],
         (0, 0, 1), LColor(0.9, 0.2, 0.2, 1)),  # 天井
        ([(-hl, -hw, 0), (-hl, -hw, hh), (hl, -hw, hh), (hl, -hw, 0)],
         (0, -1, 0), LColor(0.8, 0.15, 0.15, 1)),  # 前
        ([(hl, hw, 0), (hl, hw, hh), (-hl, hw, hh), (-hl, hw, 0)],
         (0, 1, 0), LColor(0.8, 0.15, 0.15, 1)),  # 後
        ([(hl, -hw, 0), (hl, hw, 0), (hl, hw, hh), (hl, -hw, hh)],
         (1, 0, 0), LColor(0.75, 0.12, 0.12, 1)),  # 右
        ([(-hl, hw, 0), (-hl, -hw, 0), (-hl, -hw, hh), (-hl, hw, hh)],
         (-1, 0, 0), LColor(0.75, 0.12, 0.12, 1)),  # 左
    ]
    ...
    for elem, ep, offset, rev in zip(elements, profiles, offsets, rev_flags):
        ...
        for i, (wx, wy) in enumerate(pts_2d):
            ...
            if i == 0 and points:
                ...
                z = ep.elev_at(
                    dist - offset if not rev else L - (dist - offset))
...
def build_road_mesh(centerline: list[tuple],
    ...
    road_color = color_override if color_override else LColor(
        0.25, 0.25, 0.25, 1)
...
def build_piers(centerline: list[tuple], half_width: float,
    ...
    def add_pier(cx, cy, z_top, nx_v, ny_v):
        ...
        for side in (-1, 1):
            ...
            # 底面4点
            b = [
                (ox + dx * nx_v * hw + dy * tx_v * hw,
                 oy + dx * ny_v * hw + dy * ty_v * hw, 0.0)
